...
function usercom_widget_js() {
	if ( get_option( 'UserEngageScript_domain' ) ) {
		wp_enqueue_script( 'script_widget', 'https://'. esc_html( get_option( 'UserEngageScript_domain' ) ) .'/widget.js', array(), null );
		wp_enqueue_script( 'script_ue', plugin_dir_url( __FILE__ ) . 'assets/js/ue.js', array( 'jquery' ), 1.1, true );
	} else {
		wp_enqueue_script( 'script_widget', 'https://app.userengage.com/widget.js', array(), null );
		wp_enqueue_script( 'script_ue', plugin_dir_url( __FILE__ ) . 'assets/js/ue.js', array( 'jquery' ), 1.1, true );
	}
	wp_script_add_data( 'script_widget', 'html_attrs', [
		'data-cfasync' => 'false'
	] );
}

function add_attribute_to_script_tag($tag, $handle) {
	if ($handle !== 'script_widget') return $tag;
